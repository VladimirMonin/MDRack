...
class SQLiteResourceStore:
    ...
    def _preflight(self, batch: object) -> dict[str, Any]:
        ...
        for representation in batch.representations:
            if (
                representation.resource_id != resource.resource_id
                or representation.representation_id in representation_ids
            ):
                raise ValueError("invalid representation ownership or identity")
            representation_ids.add(representation.representation_id)
            representation_modalities[representation.representation_id] = representation.modality
            _validate_token_pair(representation.token_count, representation.token_count_kind)
            representation_values.append(
                (
                    require_non_empty(representation.representation_id, "representation_id"),
                    ...
                    representation.token_count,
                    representation.token_count_kind,
                    _encode_mapping(representation.metadata, "representation.metadata"),
                )
            )
        unit_values = []
        fts_values = []
        ...
        for unit in batch.units:
            locator_kind, locator_json, _unused = _locator_parts(unit.evidence_locator, "evidence_locator")
            ordinal_key = (unit.representation_id, unit.ordinal)
            if (
                unit.resource_id != resource.resource_id
                or unit.unit_id in unit_ids
                or unit.representation_id not in representation_ids
                or ordinal_key in ordinals
                or representation_modalities[unit.representation_id] != unit.modality
            ):
            ...
            _validate_token_pair(unit.token_count, unit.token_count_kind)
            unit_ids.add(unit.unit_id)
            ordinals.add(ordinal_key)
            has_text = isinstance(unit.text, str) and bool(unit.text.strip())
            unit_has_text[unit.unit_id] = has_text
            if has_text:
                fts_values.append((unit.unit_id, unit.text))
            unit_values.append(
                (
                    require_non_empty(unit.unit_id, "unit_id"),
                    require_non_empty(unit.resource_id, "resource_id"),
                    require_non_empty(unit.representation_id, "representation_id"),
                    require_non_empty(unit.unit_kind, "unit_kind"),
                    require_non_empty(unit.modality, "modality"),
                    None if unit.text is None else require_utf8_encodable(unit.text, "text"),
                    locator_kind,
                    locator_json,
                    unit.ordinal,
                    unit.token_count,
                    unit.token_count_kind,
                    _encode_mapping(unit.metadata, "unit.metadata"),
                )
            )
        ...
        for space_record in batch.spaces:
            ...
            if type(space_record.dimensions) is not int or space_record.dimensions < 1:
            ...
            space_values.append(
                (
                    ...
                    space_record.dimensions,
                    require_non_empty(space_record.metric, "metric"),
                    require_non_empty(space_record.fingerprint, "fingerprint"),
                    _encode_mapping(space_record.metadata, "space.metadata"),
                )
            )
        vector_values = []
        vector_keys: set[tuple[str, str]] = set()
        vector_units: set[str] = set()
        for vector_record in batch.vectors:
            vector_key = (vector_record.unit_id, vector_record.space_id)
            if (
                vector_key in vector_keys
                or vector_record.unit_id not in unit_ids
            ...
            vector_values.append(
                (
                    vector_record.unit_id,
                    vector_record.space_id,
                    encoded,
                    _now(),
                )
            )
        ...
        for facet_assignment in batch.facets:
            if facet_assignment.resource_id != resource.resource_id:
                raise ValueError("facet belongs to another resource")
            namespace = require_non_empty(facet_assignment.facet.namespace, "facet.namespace")
            value = require_non_empty(facet_assignment.facet.value, "facet.value")
            origin = require_non_empty(facet_assignment.origin, "facet.origin")
            producer = require_optional_non_empty(
                facet_assignment.producer_fingerprint,
                "producer_fingerprint",
            )
